  ...
  return (
    <div className="min-h-screen bg-[#050608] text-white selection:bg-cyan-500/30">
      ...
      <div className="max-w-7xl mx-auto px-6 py-12">
        ...
        <Link to="/" className="flex items-center gap-2 text-cyan-500/50 hover:text-cyan-400 transition-all mb-8 text-[10px] font-black uppercase tracking-[0.2em] group">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 mb-12">
          <div className="flex items-center gap-5">
            <div className="hidden sm:flex p-4 bg-cyan-500/10 rounded-2xl border border-cyan-500/20 text-cyan-500 shadow-[0_0_15px_rgba(6,182,212,0.1)]">
              <Database size={32} />
            </div>
            <div>
              <h1 className="text-4xl font-black uppercase tracking-tighter italic">
                Central<span className="text-cyan-500">_Archive</span>
              </h1>
          ...
          <div className="relative w-full md:w-96 group">
             {/* Input Glow */}
             <div className="absolute inset-0 bg-cyan-500/5 blur-2xl opacity-0 group-hover:opacity-100 transition-opacity"></div>
             
             <input 
               className="w-full bg-[#0a0c10] border-2 border-white/5 rounded-2xl py-4 pl-12 pr-4 outline-none focus:border-cyan-500/40 text-gray-200 font-mono text-xs transition-all shadow-2xl relative z-10"
             ...
             />
             <Search size={18} className="absolute left-4 top-4 text-cyan-500/50 z-20" />
          </div>
        </div>

        {/* Results Grid with Tech Styling */}
        ...
        {!loading && exams.length > 0 && (
          <div className="mt-20 pt-8 border-t border-white/5 flex justify-between items-center text-[9px] font-black text-gray-600 uppercase tracking-widest">
            ...
            <span className="text-cyan-950 font-mono">{exams.length} SECTOR_ENTRIES_LOADED</span>
          </div>
        )}
      </div>
    </div>
